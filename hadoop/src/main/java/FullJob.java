...
public class FullJob extends Configured implements Tool {
    public static class FullMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public static class FullReducer extends Reducer<Text, Text, Text, Text> {
        ...
        protected void reduce(Text link, Iterable<Text> nums, Context context) throws IOException, InterruptedException {
            ...
            context.write(link, new Text(""+CTR+"\t"+CTRf+"\t"+CTRl+"\t"+Mpos+"\t"+MCpos+"\t"+Mtime+s+"\t"+probOnlyClick+"\t"+probLastClick+"\t"+avgClicksBefore+"\t"+avgClicksAfter+"\t"+Math.log1p(CTR_show)+"\t"+Math.log1p(CTR_click)+"\t"+CTRNotFirst+"\t"+CTR5+"\t"+PBM+"\t"+Meanpos_ifClick+"\t"+AvgTime+s_));
        }
    }

    private Job getJobConf(String input, String output) throws IOException {
        Job job = Job.getInstance(new Configuration());
        job.setJarByClass(FullJob.class);
    ...
    static public void main(String[] args) throws Exception {
        int ret = ToolRunner.run(new FullJob(), args);
        System.exit(ret);
    }
}