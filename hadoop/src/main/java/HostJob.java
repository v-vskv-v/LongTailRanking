...
public class HostJob extends Configured implements Tool {
    public static class HostMapper extends Mapper<LongWritable, Text, Text, Text> {
        HostsExtractor linksExtractor;

        @Override
        protected void setup(Context context) throws IOException, InterruptedException {
            super.setup(context);
            linksExtractor = new HostsExtractor(context);
        }
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            if (arr.length == 4) {
                ...
                for (int i = 0; i < links_clicked.length;i++) {
                    links_clicked[i] = links_clicked[i].split("/").length < 3 ? links_clicked[i] : links_clicked[i].split("/")[2];
                    links_clicked[i] = links_clicked[i].startsWith("www.") ? links_clicked[i].replaceFirst("www.", "") : links_clicked[i];
                }
                int pos_ = 0;
                for(String link: links_showed) {
                    ...
                    link = link.split("/").length < 3 ? link : link.split("/")[2];
                    link = link.startsWith("www.") ? link.replaceFirst("www.", "") : link;
                    ...
                    context.write(new Text(""+linksExtractor.ids.get(link)),new Text(""+pos_+"\t"+tmp+"\t"+flag_first+"\t"+flag_last+"\t"+time_watch+"\t"+onlyClickflag+"\t"+links_clicked.length));
                    pos_ += 1;
                }
            } else {
                int pos_ = 0;
                ...
                for(String link: links_showed) {
                    ...
                    context.write(new Text(""+linksExtractor.ids.get(link)), new Text(""+pos_+"\t"+tmp+"\t"+flag_first+"\t"+flag_last+"\t"+time_watch+"\t"+onlyClickflag+"\t"+0));
                    pos_ += 1;
                }
            }
        }
    ...
    public static class HostReducer extends Reducer<Text, Text, Text, Text> {
    ...
    private Job getJobConf(String input, String output) throws IOException {
        ...
        job.setJarByClass(HostJob.class);
    ...
    static public void main(String[] args) throws Exception {
        int ret = ToolRunner.run(new HostJob(), args);
        System.exit(ret);
    }
}